function jsonpRequest(src, cb, cbKey){
	...
	function initScript(src, cb, cbKey){ var target = document.createElement('script'); target.type = 'text/javascript'; target.src = src + getCallback(cb, cbKey, target); return target; }
	
	function getDocumentHead(){ return document.head || document.getElementsByTagName('head')[0]; }
	function getCallback(cb, cbKey, target){ var cbName = 'jsonPCallback' + (new Date().getTime()), closure = getClosure(cb, cbName, target); window[cbName] = closure; return '&' + cbKey + '=' + cbName; }
	function getClosure(cb, cbName, target){ var closure = function(cb, cbName, target){ return function(e){ cb(e); window[cbName] = undefined; removeScript(target); } }(cb, cbName, target); return closure; }
};
